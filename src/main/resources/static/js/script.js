...
async function confirmDelete() {
    ...
    try {
        ...
        const url = `${baseUrl}/tarefas1-1ruk9xyn.b4a.run/tarefas/excluir/${tarefaIdToDelete}`;
...
document.querySelectorAll('.btn-up').forEach(button => {
    button.addEventListener('click', async (e) => {
        ...
        const response = await fetch(`https://tarefas1-1ruk9xyn.b4a.run/tarefas/subir/${taskId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
...
document.querySelectorAll('.btn-down').forEach(button => {
    button.addEventListener('click', async (e) => {
        const taskId = e.target.getAttribute('data-id');
        const response = await fetch(`https://tarefas1-1ruk9xyn.b4a.run/tarefas/descer/${taskId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
